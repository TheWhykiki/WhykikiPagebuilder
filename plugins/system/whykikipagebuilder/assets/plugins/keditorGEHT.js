...
(function ($) {
    ...
    KEditor.DEFAULTS = {
        ...
        onContentChanged: function (event, contentArea) {
            ...
            var contentValue = contentArea[0].innerHTML;
            window.sessionStorage.setItem('history[' + historyStep +']',contentValue);
            if(historyStep >= 5){
                historyStep = 0;
            }
            ...
            if(history1 = ''){
                window.sessionStorage.setItem('history1',contentValue);
                history1 = window.sessionStorage.getItem('history1');
                console.log(1);
            }
            else{
                window.sessionStorage.setItem('history2',history1);
                window.sessionStorage.setItem('history1',contentValue);
                history1 = window.sessionStorage.getItem('history1');
                history2 = window.sessionStorage.getItem('history2');
                console.log(2);
            }
    ...
    KEditor.prototype.initContainer = function (contentArea, container) {
        ...
        if (!container.hasClass('keditor-initialized-container') || !container.hasClass('keditor-initializing-container')) {
            ...
            /**********************************************************************************************************************/
            /* Setup container type multi slider and add new edit form */
            /**********************************************************************************************************************/

            var multitype = container.find('[data-multipletype="slider"]');


            /*
            var toolbar = '<div class="keditor-toolbar keditor-toolbar-component">' +
                '               <a href="javascript:void(0);" class="keditor-ui btn-component-reposition"><i class="fa fa-arrows"></i></a>' +
                '               <a href="javascript:void(0);" class="keditor-ui btn-component-setting"><i class="fa fa-cog"></i></a>' +
                '               <a href="javascript:void(0);" class="keditor-ui btn-component-duplicate"><i class="fa fa-files-o"></i></a>' +
                '               <a href="javascript:void(0);" class="keditor-ui btn-component-delete"><i class="fa fa-times"></i></a>' +
                '           </div>';
            */
            ...
            function generateCarouselItem(counter){
                ...
                for(i = 0; i < counter; i++){
                    ...
                    carItem +=           '<div class="carousel-item keditor-container-content ui-sortable ' + activeVar +'" data-type="container-content" id="keditor-container-content-' + randomNumber + '">' +
            ...
            $(container[0].id + '.btn-container-setting').on('click',function(){
                $('#keditor-setting-title').html('Slider Einstellungen');
                $('#keditor-setting-forms').html(
                    ...
                    '           <button class="btn btn-primary testerButton">Tester</button>' +
                    '       </div>' +
                    '   </div>' +
                    '</div>');
            });
            ...
            if($(multitype).length > 0){
                ...
                $('#keditor-setting-forms').find('.txt-bg-color').on('change', function () {


                    body.removeClass('opened-keditor-setting');
                    $('#keditor-setting-forms').html('');
                    var tester = containerContents;
                    ...
                    $('#' + containerID).html('<section class="keditor-ui keditor-container-inner"><div class="row">' +
                        '        <div class="col-sm-12" data-multipletype="slider">' +
                        '' +
                        '            <div id="carouselExampleControls" class="carousel slide" data-ride="carousel">' +
                        '                <div class="carousel-inner">' +
                        generateCarouselItem(counter) +
