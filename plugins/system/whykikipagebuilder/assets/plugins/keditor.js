...
(function ($) {
    ...
    console.log('ConstParams: ' + pagebuilderParams);
    console.log(pagebuilderParams['photoSettings']);
    ...
    KEditor.DEFAULTS = {
        ...
        tabContainersText: 'Raster',
        tabContainersTitle: 'Raster',
        tabComponentsText: 'Komponenten',
        tabComponentsTitle: 'Komponenten',
        tabTooltipEnabled: false,
        extraTabs: null,
        defaultComponentType: 'blank',
        sidebarContainer: null,
        snippetsUrl: '../plugins/system/whykikipagebuilder/snippets/snippets.html',
        snippetsTooltipEnabled: false,
        snippetsTooltipPosition: 'left',
        snippetsFilterEnabled: false,
    ...
    KEditor.prototype.renderSnippets = function (resp) {
        ...
        var snippetsContainerHtmlLeft = '';
        var snippetsContainerHtmlRight = '';
        var snippetsComponentHtmlLeft = '';
        var snippetsComponentHtmlRight = '';
        ...
        $('<div />').html(resp).find('> div').each(function (i) {
            ...
            var previewHeadline = snippet.attr('data-previewheadline');
            var previewUrl = snippet.attr('data-preview');
            var type = snippet.attr('data-type');
            var dataTab = snippet.attr('data-tab');
            var editorSide = snippet.attr('data-editorside');
            var title = snippet.attr('data-keditor-title');
            var snippetHtml = '';
            var categories = snippet.attr('data-keditor-categories') || '';

            snippetHtml += '<section class="keditor-ui keditor-snippet" data-snippet="#keditor-snippet-' + i + '" data-type="' + type + '" data-editorSide="' + editorSide + '" ' + (options.snippetsTooltipEnabled ? 'data-toggle="tooltip" data-placement="' + options.snippetsTooltipPosition + '"' : '') + ' title="' + title + '" data-keditor-categories="' + categories + '">';
            snippetHtml += '   <h4 class="previewHeadline">' + previewHeadline + '</h4>';
            snippetHtml += '   <img class="keditor-ui keditor-snippet-preview" src="' + previewUrl + '" />';
            snippetHtml += '</section>';

            categories = categories.split(options.snippetsCategoriesSeparator);

            if (dataTab === 'container' & editorSide === 'left') {
                snippetsContainerHtmlLeft += snippetHtml;
                self.snippetsContainerCategories = self.snippetsContainerCategories.concat(categories);
            }else if (dataTab === 'container' & editorSide === 'right') {
                snippetsContainerHtmlRight += snippetHtml;
                self.snippetsContainerCategories = self.snippetsContainerCategories.concat(categories);
            }
            else if (dataTab === 'component' & editorSide === 'left') {
                snippetsComponentHtmlLeft += snippetHtml;
                self.snippetsComponentCategories = self.snippetsComponentCategories.concat(categories);
            }
            else if (dataTab === 'component' & editorSide === 'right') {
                snippetsComponentHtmlRight += snippetHtml;
                self.snippetsComponentCategories = self.snippetsComponentCategories.concat(categories);
            }
        ...
        body.find('#keditor-snippets-list').html(
            ...
            '       <div class="keditor-ui keditor-tab-content active" id="keditor-container-snippets-tab">' +
            '           <div class="tabsFlexContainer">' +
            '               <div class="keditor-ui keditor-tab-content-inner leftSide">' + snippetsContainerHtmlLeft + '</div>' +
            '               <div class="keditor-ui keditor-tab-content-inner rightSide">' + snippetsContainerHtmlRight + '</div>' +
            '           </div>' +
            '       </div>' +
            '       <div class="keditor-ui keditor-tab-content" id="keditor-component-snippets-tab">' +
            '           <div class="tabsFlexContainer">' +
            '               <div class="keditor-ui keditor-tab-content-inner leftSide">' + snippetsComponentHtmlLeft + '</div>' +
            '               <div class="keditor-ui keditor-tab-content-inner rightSide">' + snippetsComponentHtmlRight + '</div>' +
            '           </div>' +
            '       </div>' +
            '</div>'
    ...
    KEditor.prototype.showSettingPanel = function (target) {
        ...
        if (isComponent) {
            ...
            body.find('#keditor-setting-title').html();
    ...
    KEditor.prototype.initContainer = function (contentArea, container) {
        ...
        if (!container.hasClass('keditor-initialized-container') || !container.hasClass('keditor-initializing-container')) {
            ...
            var multitypeSlider = container.find('[data-multipletype="slider"]');
            var multitypeSlider2 = container.find('[data-multipletype="slider2"]');

            //multitypeSlider.find('.carousel').attr


            if (options.containerSettingEnabled === true) {
                settingBtn = '<a href="javascript:void(0);" class="keditor-ui btn-container-setting">' + options.btnSettingContainerText + '</a>';
            }
            else if($(multitypeSlider).length > 0){
                settingBtn = '<a href="javascript:void(0);" class="keditor-ui btn-container-settingSlider">' + options.btnSettingContainerText + '</a>';
            }
            else if($(multitypeSlider2).length > 0){
                settingBtn = '<a href="javascript:void(0);" class="keditor-ui btn-container-settingSlider2">' + options.btnSettingContainerText + '</a>';
            }
            ...
            /**********************************************************************************************************************/
            /* Generate carousel items based on counter */
            /**********************************************************************************************************************/

            function generateCarouselItem(counter){
            ...
            if($(multitypeSlider).length > 0) {

                var sliderID1 = multitypeSlider[0].id;

                console.log(sliderID1);
                $(' .btn-container-settingSlider').on("click", function (e) {
                    e.preventDefault();
                    console.log(sliderID1);
                    ...
                    $('#keditor-setting-title').html('Slider Einstellungen1');
                    $('#keditor-setting-forms').html(
                        '<div class="form-horizontal formSlider">' +
                        '   <div class="form-group">' +
                        '       <div class="col-sm-12">' +
                        '           <label>Anzahl Slides</label>' +
                        '           <input type="text" class="form-control slidesCount1" />' +
                        '           <button class="btn btn-primary saveSlider1">Speichern</button>' +
                ...
                $(document).one('sliderinit', function (e) {
                    ...
                    $('.addSlider1').on('click', function () {

                        sliderAddCounter1 = $('.slidesAddCount1').val();
                        console.log(sliderID1);
                        // Generate carousel and add number of items

                        $('#' + sliderID1 + ' #carousel1 .carousel-inner').prepend(
                            generateCarouselItem(sliderAddCounter1)
                        );
                        $(document).trigger('contentchange');
                        $('body').removeClass('opened-keditor-setting');
                        $('#keditor-setting-forms').html('');
                    ...
                    $('.saveSlider1').on('click', function () {
                        ...
                        $('body').removeClass('opened-keditor-setting');
                        $('#keditor-setting-forms').html('');
                        $("#carousel1 .carousel-inner .carousel-item:first").addClass("active");
                    });
                });
            }

            if($(multitypeSlider2).length > 0) {

                var sliderID2 = multitypeSlider2[0].id;

                console.log(sliderID2);
                $(' .btn-container-settingSlider2').on("click", function (e) {
                    ...
                    $('#keditor-setting-title').html('Slider Einstellungen2');
                    $('#keditor-setting-forms').html(
                        '<div class="form-horizontal formSlider">' +
                        '   <div class="form-group">' +
                        '       <button class="col-sm-12">' +
                        '           <label>Anzahl Slides</label>' +
                        '           <input type="text" class="form-control slidesCount2" />' +
                        '           <button class="btn btn-primary saveSlider2">Anzahl setzen</button>' +
                        '           <input type="text" class="form-control slidesAddCount2" />' +
                        '           <button class="btn btn-primary addSlider2">Slides hinzufügen</button>' +
                        '           <p>Nach dem Einstellen einer neuen Anzahl, bitte den Beitrag einmal speichern. <br> Wenn der Beitrag nicht gespeicchert wird, können die Bilder im Slider nicht verändert werden.</p>' +
                        '       </div>' +
                        '   </div>' +
                        '</div>');

                    $(document).trigger('sliderinit2');
                });
                ...
                $(document).one('sliderinit2', function (e) {
                    e.preventDefault();
                    e.stopImmediatePropagation();

                    //

                    $('.addSlider2').on('click', function () {

                        sliderAddCounter2 = $('.slidesAddCount2').val();
                        console.log(sliderID2);
                        // Generate carousel and add number of items

                        $('#' + sliderID2 + ' #carousel2 .carousel-inner').prepend(
                            generateCarouselItem(sliderAddCounter2)
                        );
                        $(document).trigger('contentchange');
                        $('body').removeClass('opened-keditor-setting');
                        $('#keditor-setting-forms').html('');
                    ...
                    $('.saveSlider2').on('click', function () {

                        sliderCounter2 = $('.slidesCount2').val();
                        console.log(sliderID2);
                        // Generate carousel and add number of items

                        $('#' + sliderID2).html('<div id="carousel2" class="carousel slide" data-ride="carousel">' +
                            '                <div class="carousel-inner">' +
                            generateCarouselItem(sliderCounter2) +
                        ...
                        $('body').removeClass('opened-keditor-setting');
                        $('#keditor-setting-forms').html('');
                        $("#carousel2 .carousel-inner .carousel-item:first").addClass("active");
                    });
                });
            }
    ...
    KEditor.prototype.initKEditorClicks = function () {
        ...
        body.on('click', '.btn-container-settings', function (e) {
            e.preventDefault();
    ...
    KEditor.prototype.getContainerContentNoSection = function (container, isNested) {
        flog('getContainerContent - isNested=' + isNested, container);
        ...
        containerInner.find('[data-type=container-content]').not(isNested ? '' : '.keditor-sub-container-content').each(function () {
            ...
            containerContent.children().each(function () {
                ...
                if (child.is('.keditor-component')) {
                    child.replaceWith(self.getComponentContentWithoutSection(child));
                } else if (child.is('.keditor-sub-container')) {
                    child.replaceWith(self.getContainerContentNoSection(child, true));
                }
            });
        });
    ...
    KEditor.prototype.getComponentContentWithoutSection = function (component) {
        flog('getComponentContent', component);
    ...
    KEditor.prototype.getContentWithoutSection = function (inArray) {
        var self = this;
        var options = self.options;
        var result = [];
        var target = options.iframeMode ? self.body : self.element;
        ...
        target.find('.keditor-content-area').each(function () {
            ...
            $(this).children('.keditor-container').each(function () {
                var container = $(this);
                html += self.getContainerContentNoSection(container);
            });
